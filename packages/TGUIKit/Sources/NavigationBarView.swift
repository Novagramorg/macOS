...
public class NavigationBarView: View {
    ...
    private var leftView: BarView = BarView(frame: NSRect.zero)
    private var centerView: BarView = BarView(frame: NSRect.zero)
    private var rightView: BarView = BarView(frame: NSRect.zero)
   ...
   //     self.autoresizingMask = [.width]
    ...
    /// Set to `false` by controllers that draw their own glass chrome inside the
    /// bar — the chat title capsule being the one that matters. Two stacked glass
    /// layers read as an opaque box instead of glass, which is exactly what 12.9
    /// avoids: its title capsule is see-through, with chat content visible behind.
    public var usesGlassBackground: Bool = true {
        didSet {
            if usesGlassBackground != oldValue {
                updateLocalizationAndTheme(theme: presentation)
            }
        }
    }
    ...
    override public func updateLocalizationAndTheme(theme: PresentationTheme) {
        ...
        // Novagram: 12.9 has NO navigation bar chrome — no fill, no glass, no
        // hairline. The bar is invisible and its buttons float directly on the
        // window backdrop as circular icon buttons. Verified against the official
        // client's Appearance screen, where the title and the ‹ / ⋯ buttons sit on
        // the same colour as the page behind them.
    ...
    override public func layout() {
        super.layout()
        self.glassView?.frame = bounds
        self.bottomBorder.frame = NSRect(x: 0, y: frame.height - .borderSize, width: frame.width, height: .borderSize)
...
//        }
    ...
    func layout(left: BarView, center: BarView, right: BarView, force: Bool = false) {
        ...
        if frame.height > 0 {
            ...
            left.frame = NSRect(x: startPosition, y: 0, width: leftWidth, height: frame.height)
            center.frame = NSRect(x: left.frame.maxX, y: 0, width: width - (leftWidth + rightWidth), height: frame.height)
            right.frame = NSRect(x: center.frame.maxX, y: 0, width: rightWidth, height: frame.height)
        }
    }
    ...
    func moveViews(left: BarView, center: BarView, right: BarView, direction: SwipeDirection, percent: CGFloat, animationStyle: AnimationStyle? = nil) {
        ...
        left.change(pos: NSPoint(x: floorToScreenPixels(backingScaleFactor, nLeft_to), y: left.frame.minY), animated: animationStyle != nil, duration: animationStyle?.duration ?? 0, timingFunction: animationStyle?.function ?? .linear, completion: { [weak left] completed in
            if completed && animationStyle != nil {
                left?.removeFromSuperview()
            }
        })
        center.change(pos: NSPoint(x: floorToScreenPixels(backingScaleFactor, nCenter_to), y: center.frame.minY), animated: animationStyle != nil, duration: animationStyle?.duration ?? 0, timingFunction: animationStyle?.function ?? .linear, completion: { [weak center] completed in
            if completed && animationStyle != nil {
                center?.removeFromSuperview()
            }
        })
        right.change(pos: NSPoint(x: floorToScreenPixels(backingScaleFactor, nRight_to), y: right.frame.minY), animated: animationStyle != nil, duration: animationStyle?.duration ?? 0, timingFunction: animationStyle?.function ?? .linear, completion: { [weak right] completed in
            if completed && animationStyle != nil {
                right?.removeFromSuperview()
            }
        })

        self.leftView.change(pos: NSPoint(x: floorToScreenPixels(backingScaleFactor, pLeft_to), y: self.leftView.frame.minY), animated: animationStyle != nil, duration: animationStyle?.duration ?? 0, timingFunction: animationStyle?.function ?? .linear)
        self.centerView.change(pos: NSPoint(x: floorToScreenPixels(backingScaleFactor, pCenter_to), y: self.centerView.frame.minY), animated: animationStyle != nil, duration: animationStyle?.duration ?? 0, timingFunction: animationStyle?.function ?? .linear)
        self.rightView.change(pos: NSPoint(x: floorToScreenPixels(backingScaleFactor, pRight_to), y: self.rightView.frame.minY), animated: animationStyle != nil, duration: animationStyle?.duration ?? 0, timingFunction: animationStyle?.function ?? .linear)
...
//
            ...
            pLeft.change(pos: NSPoint(x: pLeft_to, y: pLeft.frame.minY), animated: true, duration: animationStyle.duration, timingFunction: animationStyle.function)
         ...
         //   pCenter.change(pos: NSMakePoint(pCenter_to, pCenter.frame.minY), animated: true, duration: animationStyle.duration, timingFunction: animationStyle.function)
            ...
            pRight.change(pos: NSPoint(x: pRight_to, y: pRight.frame.minY), animated: true, duration: animationStyle.duration, timingFunction: animationStyle.function)

            // new
            if !left.isHidden {
                left.change(opacity: 1.0, duration: animationStyle.duration, timingFunction: animationStyle.function)
            }
            left.change(pos: NSPoint(x: nLeft_to, y: left.frame.minY), animated: true, duration: animationStyle.duration, timingFunction: animationStyle.function)

            if !center.isHidden {
                center.change(opacity: 1.0, duration: animationStyle.duration, timingFunction: animationStyle.function)
            }

            center.change(pos: NSPoint(x: nCenter_to, y: center.frame.minY), animated: true, duration: animationStyle.duration, timingFunction: animationStyle.function)

            if !right.isHidden {
                right.change(opacity: 1.0, duration: animationStyle.duration, timingFunction: animationStyle.function)
            }
            right.change(pos: NSPoint(x: nRight_to, y: right.frame.minY), animated: true, duration: animationStyle.duration, timingFunction: animationStyle.function)
