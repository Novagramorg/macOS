...
final class MenuView: Control, TableViewDelegate {
    ...
    private let backgroundView: View
    // Novagram: was NSVisualEffectView — AdaptiveGlassEffectView keeps that exact
    // behaviour as its legacy backend and swaps in real Liquid Glass on macOS 26,
    // matching official Telegram 12.9 where context menus are glass.
    ...
    required init(frame frameRect: NSRect) {
        ...
        self.visualView.wantsLayer = true
        self.visualView.legacyState = .active
        ...
        // 12.9 uses a noticeably rounder menu than the legacy 10pt corner
        let radius: CGFloat = isLiquidGlassAvailable ? 14 : 10
        self.layer?.cornerRadius = radius
        self.visualView.cornerRadius = radius
    ...
    func makeSize(presentation: AppMenu.Presentation, screen: NSScreen, maxHeight: CGFloat? = nil, appearMode: AppMenu.AppearMode) {
        ...
        if presentation.colors.isDark {
            visualView.legacyMaterial = .dark
        } else {
            visualView.legacyMaterial = .light
        }
        effectiveSize = max
    ...
    func merge(menu: ContextMenu, presentation: AppMenu.Presentation, interaction: AppMenuBasicItem.Interaction) {
        self.observation = menu.observe(\._items, options: [.new], changeHandler: { [weak self] _, value in
...
final class AppMenuController: NSObject {
    ...
    private var previousCopyHandler: (() -> Void)?
    ...
    func initialize() {
        ...
        case .click:
            self.parent?.set(mouseHandler: { _ in
                isInteracted = true
                return .rejected
            }, with: self, for: .leftMouseDown, priority: .modal)

            self.parent?.set(mouseHandler: { [weak self] _ in
                if isInteracted {
                    self?.closeAll()
                }
        ...
        case .hover:
            if let window = event.window {
                if let parentView = parentView, let superview = parentView.superview {
                    ...
                    if s_v_rect.contains(s_m_point) || mouseInMenu {
    ...
    private var foundItem: TableRowItem?
    ...
    private func presentSubmenu(_ menu: ContextMenu, parentView: Window, for id: Int64) {
        ...
        var point = parentItemView.convert(NSPoint(x: parentItemView.frame.width - 5, y: -parentItemView.frame.height), to: nil)
    ...
    func activate(event: NSEvent, view: NSView, animated: Bool) {
        ...
        case .hover:
            if let parentView = parentView, let superview = parentView.superview {
                ...
                rect = CGRect(origin: NSPoint(x: v_rect.minX, y: v_rect.minY - 5), size: view.frame.size)
            } else {
        ...
        if let window = menu.topWindow {
            ...
            let rect = NSRect(x: rect.maxX - width + 40, y: rect.maxY - 25, width: width, height: window.frame.height)
            window.setFrame(rect, display: true)
            window.makeKeyAndOrderFront(nil)

            let view = window.contentView!.subviews.first!

            view.frame = NSRect(x: rect.focus(view.frame.size).minX, y: 0, width: view.frame.width, height: view.frame.height)

            window.contentView?.layer?.animateAlpha(from: 0.1, to: 1, duration: 0.2)
            window.contentView?.layer?.animateScaleSpringFrom(anchor: NSPoint(x: anchor.x, y: rect.height / 2), from: 0.1, to: 1, duration: 0.2, bounce: false)
