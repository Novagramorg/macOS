...
final class Auth_LoginHeader: View {
    // Fenixuz: app's own phoenix icon instead of the Telegram airplane.
    private let logo: NSImageView = NSImageView(frame: Auth_Insets.logoSize.bounds)
    ...
    private func updateLottie() {
        // Fenixuz: render the app's own icon (phoenix) instead of the Telegram airplane.
        self.logo.image = NSApp.applicationIconImage
        self.logo.imageScaling = .scaleProportionallyUpOrDown
        self.logo.setFrameSize(Auth_Insets.logoSize)
    }
...
final class Auth_PhoneInput: View, NSTextFieldDelegate {
    ...
    private func updatePlaceholder() {
        ...
        if !number.isEmpty {
            attr.addAttribute(.foregroundColor, value: NSColor.clear, range: NSRange(location: 0, length: min(number.length, text.length)))
        }
    ...
    override func layout() {
        ...
        self.separator.frame = focus(NSSize(width: frame.width - 20, height: .borderSize))
        ...
        placeholder.setFrameOrigin(NSPoint(x: 10 + codeText.frame.width + 10 + 2, y: frame.height - floor(frame.height / 2 - placeholder.frame.height/2) + 2))

        country.setFrameOrigin(NSPoint(x: 10, y: 12))
        country_overlay.frame = NSRect(x: 0, y: 0, width: frame.width, height: frame.height / 2)
        nextView.setFrameOrigin(NSPoint(x: frame.width - 10 - nextView.frame.width, y: floor((frame.height / 2 - nextView.frame.height)/2)))
    }

    func update(selected: Country?, update: Bool, updateCode: Bool = true) {
...
final class Auth_PhoneNumberView: View {
    ...
    override func updateLocalizationAndTheme(theme: PresentationTheme) {
        super.updateLocalizationAndTheme(theme: theme)
        let theme = theme as! TelegramPresentationTheme
        self.header.setFrameSize(NSSize(width: frame.width, height: self.header.height))
