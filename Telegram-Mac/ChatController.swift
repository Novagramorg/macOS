...
class ChatCustomLinkContent {
    ...
    var editName: (() -> Void)?
    var interfaceUpdate: (() -> Void)?
    var saveText: ((ChatTextInputState) -> Void)?
...
class ChatControllerView: View, ChatInputDelegate {
    ...
    fileprivate var updateFloatingPhotos: ((ScrollPosition, Bool) -> Void)?
    ...
    private let gradientMaskView = BackgroundGradientView(frame: NSRect.zero)
    ...
    func updateBackground(_ mode: TableBackgroundMode, navigationView: NSView?, isStandalone: Bool) {
        if mode != theme.controllerBackgroundMode || isStandalone {
            if let navigationView = navigationView, backgroundMode != mode, self.backgroundView == nil {
                let point = NSPoint(x: 0, y: -frame.minY)
    ...
    required init(frame frameRect: NSRect, chatInteraction: ChatInteraction) {
        ...
        tableView = TableView(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: frameRect.height - 50), isFlipped: false)

        inputView = ChatInputView(frame: NSRect(x: 0, y: tableView.frame.maxY, width: frameRect.width, height: 50), chatInteraction: chatInteraction)
...
//        floatingPhotosView.backgroundColor = .random
        ...
        tableView.addScroll(listener: TableScrollListener(dispatchWhenVisibleRangeUpdated: false, { [weak self] _ in
       ...
       // updateLocalizationAndTheme(theme: theme)
        ...
        tableView.set(stickClass: ChatDateStickItem.self, handler: { _ in

        })

        tableView.setSecondary(stickClass: ChatTopicSeparatorItem.self, handler: { _ in
    ...
    var tableRect: NSRect {
        ...
        let tableRect = NSRect(x: 0, y: header.state.toleranceHeight, width: frame.width, height: tableHeight)
    ...
    func updateFrame(_ frame: NSRect, transition: ContainedViewLayoutTransition) {
        ...
        var headerInset: NSPoint = NSPoint(x: 0, y: 0)
        ...
        if let view = inputContextHelper.accessoryView {
            transition.updateFrame(view: view, frame: NSRect(x: 0, y: frame.height - inputView.frame.height - view.frame.height, width: frame.width, height: view.frame.height))
        }
        if let currentView = header.currentView {
            header.measure(frame.width - headerInset.x)

            transition.updateFrame(view: currentView, frame: NSRect(x: headerInset.x, y: headerInset.y, width: frame.width - headerInset.x, height: currentView.frame.height))
        ...
        let inputRect = NSRect(x: 0, y: inputY, width: frame.width, height: inputHeight)
        transition.updateFrame(view: inputView, frame: inputRect)
        inputView.updateLayout(size: NSSize(width: frame.width, height: inputView.frame.height), transition: transition)
        ...
        if let backgroundView = backgroundView, let navigationView = navigationView {
            let size = NSSize(width: navigationView.bounds.width, height: navigationView.bounds.height)
            transition.updateFrame(view: backgroundView, frame: NSRect(x: 0, y: -(frame.minY - 50), width: size.width, height: navigationView.bounds.height))
        }
        ...
        if let themeSelectorView = self.themeSelectorView {
            transition.updateFrame(view: themeSelectorView, frame: NSRect(x: 0, y: frame.height - themeSelectorView.frame.height, width: frame.width, height: themeSelectorView.frame.height))
        }
        ...
        if let monoforum_VerticalView {
            transition.updateFrame(view: monoforum_VerticalView, frame: NSRect(x: 0, y: 0, width: monoforum_VerticalView.frame.width, height: tableRect.height))
        ...
        if let monoforum_HorizontalView {
            transition.updateFrame(view: monoforum_HorizontalView, frame: NSRect(x: 0, y: 0, width: frame.width, height: monoforum_HorizontalView.frame.height))
    ...
    func change(state: ChatControllerViewState, animated: Bool) {
        ...
        if state != self.state {
            ...
            case .progress:
                if progressView == nil {
                    self.progressView = ProgressIndicator(frame: NSRect(x: 0, y: 0, width: 30, height: 30))
        ...
        if let state {
            ...
            case .horizontal:
                if let view = monoforum_VerticalView {
                    performSubviewPosRemoval(view, pos: NSPoint(x: -view.frame.width, y: 0), animated: animated)
                ...
                if let view = self.monoforum_HorizontalView {
                    ...
                    current = MonoforumHorizontalView(frame: NSRect(x: 0, y: 0, width: frame.width, height: 40))
                    ...
                    if animated {
                        ...
                        current.layer?.animatePosition(from: NSPoint(x: 0, y: -current.frame.height), to: .zero)
                    }
                }
            ...
            case .vertical:
                if let view = monoforum_HorizontalView {
                    performSubviewPosRemoval(view, pos: NSPoint(x: 0, y: -view.frame.height), animated: animated)
                ...
                if let view = self.monoforum_VerticalView {
                    ...
                    current = MonoforumVerticalView(frame: NSRect(x: 0, y: 0, width: 80, height: tableRect.height))
                    addSubview(current, positioned: .below, relativeTo: inputView)
                    self.monoforum_VerticalView = current

                    if animated {
                        current.layer?.animateAlpha(from: 0.5, to: 1, duration: 0.2)
                        current.layer?.animatePosition(from: NSPoint(x: -current.frame.width, y: 0), to: .zero)
                    }
                }
            ...
            current.sizeToFit(.zero, NSSize(width: 80, height: 40), thatFit: true)
    ...
    private var scrollerRect: NSRect {
        return NSRect(x: frame.width - scroller.frame.width - 6, y: frame.height - inputView.frame.height - 6 - scroller.frame.height, width: scroller.frame.width, height: scroller.frame.height)
    }
...
//        subscriber.putCompletion()
        ...
        var scrollToItem: TableScrollState?
        ...
        if firstTransition, let state = scrollToItem, prevIsLoading {
            ...
            messagesViewQueue.async {
                if !cancelled.with({ $0 }) {
                    ...
                    var firstInsertedRange: NSRange = NSRange(location: 0, length: 0)

                    if !firstInsertion.isEmpty {
                        firstInsertedRange = NSRange(location: initialIndex, length: firstInsertion.count)
                    }
...
//    private var reactionManager:AddReactionManager?
    ...
    private var themeSelector: ChatThemeSelectorController?
    ...
    private struct State: Equatable {
        ...
        var presentation_emoticon: String?
        ...
        var peerStatus: PeerStatusSettings?
    ...
    private func updateFloatingPhotos(_ position: ScrollPosition, animated: Bool, currentAnimationRows: [TableAnimationInterface.AnimateItem] = []) {
        ...
        for groupped in grouppedFloatingPhotos {
            ...
            if let revealView = revealView {
                ...
                let rect = NSRect(x: 0, y: minOffset, width: revealView.frame.width, height: maxOffset - minOffset)
                if rect.contains(point) {
                    point.x += revealView.containerX
                } else if rect.contains(NSPoint(x: point.x, y: point.y + photoView.frame.height - 1)) {
                    point.x += revealView.containerX
                }
    ...
    override func viewDidLoad() {
        ...
        genericView.inputContextHelper.didScroll = { [weak self] in
            self?.genericView.updateTextInputSuggestions([], range: NSRange(location: 0, length: 0), animated: true)
        }
        ...
        case let .peer(peerId):
            ...
            _ = context.engine.peers.checkPeerChatServiceActions(peerId: peerId).start()
        case let .thread(data):
...
//        }
        ...
        let appliedTransition = historyViewTransition |> map { [weak self] transition, _, initialData, isLoading, proccesedView in
        ...
        let readHistory = combineLatest(self.maxVisibleIncomingMessageIndex.get(), canRead, self.chatLocationValue)
            |> map { messageIndex, canRead, chatLocation in
                if canRead {
                    var apply = false
                    _ = previousMaxIncomingMessageIdByNamespace.modify { dict in
                    ...
                    if apply {
                        if !hasModals(context.window) {
                            ...
                            // Fenixuz Ghost mode: applyMaxReadIndex tells the server we read up to here —
                            // that IS the read receipt ghost mode must hide. Suppress it while active.
        ...
        chatInteraction.updateFrame = { [weak self] _, transition in
...
//            self.reactionManager?.clearAndTempLock()
        ...
        let editMessage: (ChatEditState, Date?) -> Void = { [weak self] state, atDate in
            ...
            let inputState = state.inputState.subInputState(from: NSRange(location: 0, length: state.inputState.inputText.length))
        ...
        chatInteraction.sendMessage = { [weak self] silent, atDate, messageEffect in
            if let strongSelf = self, !strongSelf.nextTransaction.isExutable {
                ...
                if presentation.abilityToSend {
                    // Fenixuz Ghost mode: actively sending a message implies you read the chat, so apply
                    // the read index now (ghost only hides PASSIVE reading) — otherwise the recipient sees
                    // a reply from someone who supposedly never read their messages.
                    if FenixuzGhostMode.isActive {
                        _ = (strongSelf.maxVisibleIncomingMessageIndex.get() |> take(1) |> deliverOnMainQueue).start(next: { [weak strongSelf] index in
                            guard let strongSelf = strongSelf else { return }
                            strongSelf.context.applyMaxReadIndex(for: strongSelf.chatLocation, contextHolder: strongSelf.chatLocationContextHolder, messageIndex: index)
                        })
                    }
        ...
        chatInteraction.sendMessageMenu = { [weak self] fromEffect in
            ...
            return reactions |> map { [weak self] reactions in
                ...
                let attributed = parseMarkdownIntoAttributedString(aboveText, attributes: .init(body: .init(font: .normal(.text), textColor: color), bold: .init(font: .medium(.text), textColor: color), link: .init(font: .normal(.text), textColor: link), linkAttribute: { _ in
                ...
                let rect = NSRect(x: 0, y: 0, width: w_width, height: 40 + 20 + aboveLayout.layoutSize.height + 4)
                ...
                let view = ContextAddReactionsListView(frame: rect, context: context, list: available, add: { value, _, fromRect in
                    switch value {
                    case let .custom(fileId):
        ...
        chatInteraction.unarchive = { [weak self] in
            ...
            let removeFlagsSignal = context.account.postbox.transaction { transaction in
                transaction.updatePeerCachedData(peerIds: [peerId], update: { _, cachedData in
                    if let cachedData = cachedData as? CachedUserData {
        ...
        chatInteraction.deleteMessages = { [weak self] messageIds in
            if let strongSelf = self, let peer = strongSelf.chatInteraction.peer {
                ...
                self?.messagesActionDisposable.set(combineLatest(context.account.postbox.messagesAtIds(messageIds) |> deliverOnMainQueue, channelAdmin.get() |> deliverOnMainQueue).start( next: { [weak strongSelf] messages, _ in
        ...
        chatInteraction.runEmojiScreenEffect = { [weak self] emoji, message, mirror, isIncoming in
            ...
            let messageId = message.id
            if strongSelf.isOnScreen {
                strongSelf.emojiEffects.addAnimation(emoji.fixed, index: nil, mirror: mirror, isIncoming: isIncoming, messageId: messageId, animationSize: NSSize(width: 350, height: 350), viewFrame: context.window.bounds, for: context.window.contentView!)
            }
        }
        ...
        chatInteraction.runReactionEffect = { [weak self] value, messageId in
            ...
            if strongSelf.isOnScreen {
                strongSelf.emojiEffects.addReactionAnimation(value, index: nil, messageId: messageId, animationSize: NSSize(width: 80, height: 80), viewFrame: context.window.bounds, for: context.window.contentView!)
            }
        }
            ...
            if let strongSelf = self, let peer = strongSelf.chatInteraction.peer {
                ...
                case .scheduled:
                    showModal(with: DateSelectorModalController(context: strongSelf.context, mode: .schedule(peer.id), selectedAt: { [weak strongSelf] date in
                        if let strongSelf = strongSelf {
                            _ = (Sender.enqueue(media: media, context: context, peerId: peerId, replyId: takeReplyId(), threadId: threadId64(), atDate: date, sendAsPeerId: currentSendAsPeerId, customChatContents: customChatContents, sendPaidMessageStars: takePaidMessageStars()) |> deliverOnMainQueue).start(completed: scrollAfterSend)
                            strongSelf.nextTransaction.set(handler: afterSentTransition)
                        }
                    }), for: strongSelf.context.window)
                case .history, .thread, .customChatContents:
                    _ = (Sender.enqueue(media: media, context: context, peerId: peerId, replyId: takeReplyId(), threadId: threadId64(), sendAsPeerId: currentSendAsPeerId, customChatContents: customChatContents, sendPaidMessageStars: takePaidMessageStars()) |> deliverOnMainQueue).start(completed: scrollAfterSend)
                    strongSelf.nextTransaction.set(handler: afterSentTransition)
                case .pinned:
                    break
                case .customLink:
        ...
        chatInteraction.sendAppFile = { [weak self] file, silent, query, schedule, collectionId in
            if let strongSelf = self, let peer = strongSelf.chatInteraction.peer, peer.canSendMessage(strongSelf.mode.isThreadMode, media: file, threadData: strongSelf.chatInteraction.presentation.threadInfo, cachedData: strongSelf.chatInteraction.presentation.cachedData) {
                ...
                let invoke: () -> Void = {
                    ...
                    func apply(_ controller: ChatController, atDate: Date?) {
                        _ = (Sender.enqueue(media: file, context: context, peerId: peerId, replyId: takeReplyId(), threadId: threadId64(), silent: silent, atDate: atDate, query: query, collectionId: collectionId, customChatContents: customChatContents, sendPaidMessageStars: takePaidMessageStars()) |> deliverOnMainQueue).start(completed: scrollAfterSend)
        ...
        chatInteraction.unpinAllMessages = { [weak self, unowned context] in
            ...
            if canManagePin {
                ...
                verifyAlert_button(for: context.window, information: strings().chatUnpinAllMessagesConfirmationCountable(count), ok: strings().chatConfirmUnpinOK, cancel: strings().modalCancel, successHandler: { [weak self] _ in
                    _ = (context.engine.messages.requestUnpinAllMessages(peerId: peerId, threadId: self?.chatLocation.threadId)
                    ...
                    case .Loading:
                        ...
                        for i in 0 ..< view.entries.count {
                            ...
                            if matches, chatLocation.threadId == entry.message.threadId || chatLocation.threadId == nil && entry.message.threadId == 1 || entry.message.threadId == context.peerId.toInt64() {
                                message = ChatPinnedMessage(messageId: entry.message.id, message: entry.message, others: view.entries.map {
                                        $0.message.id
                                    }, isLatest: i == view.entries.count - 1, index: view.entries.count - 1 - i, totalCount: view.entries.count)
                            }
                        }
                    }
                    return message
        ...
        let threadInfo: Signal<MessageHistoryThreadData?, NoError> = chatLocationValue |> mapToSignal { [weak self] chatLocation in
            switch chatLocation {
            case .peer:
                return .single(nil) |> beforeNext { [weak self] _ in
        ...
        let connectionStatus = combineLatest(queue: .mainQueue(), connecting, updating) |> deliverOnMainQueue |> beforeNext { [weak self] status, isUpdating in
            var status = status
            switch status {
        ...
        self.peerInputActivitiesDisposable.set((peerInputActivities
            |> mapToSignal { activities -> Signal<[(Peer, PeerInputActivity)], NoError> in
                ...
                previousPeerCache.with { dict in
                            ...
                            if let item = item {
                                ...
                                for animation in animations {
                                    delay(Double(animation.timeOffset), closure: { [weak strongSelf] in
                                        ...
                                        strongSelf.emojiEffects.addAnimation(emoticon.fixed, index: animation.index, mirror: mirror, isIncoming: true, messageId: messageId, animationSize: NSSize(width: 350, height: 350), viewFrame: context.window.bounds, for: context.window.contentView!)
                                    })
                                }
                            }
                        default:
    ...
    private weak var dustLayerView: DustLayerView?
...
//        self.genericView.updateHeader(chatInteraction.presentation, false, false)
    ...
    public init(context: AccountContext, chatLocation: ChatLocation, mode: ChatMode = .history, focusTarget: ChatFocusTarget? = nil, initialAction: ChatInitialAction? = nil, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>? = nil) {
        ...
        var takeTableItem: ((MessageId) -> ChatRowItem?)?
...
//        }
    ...
    private var tempImportersContext: PeerInvitationImportersContext?
    ...
    func notify(with value: Any, oldValue: Any, animated: Bool, force: Bool) {
        if let value = value as? ChatPresentationInterfaceState, let oldValue = oldValue as? ChatPresentationInterfaceState {
            ...
            if input != oldValue.effectiveInput || value.botMenu != oldValue.botMenu || force {
                ...
                if cleanup {
                    self.genericView.updateTextInputSuggestions([], range: NSRange(location: 0, length: 0), animated: animated)
                ...
                if let (updatedContextQueryState, updatedContextQuerySignal) = contextQueryResultStateForChatInterfacePresentationState(value, context: self.context, currentQuery: self.contextQueryState?.0) {
                    ...
                    var disableEditingPreview: ((String) -> Void)?
    ...
    override public func update(with state: ViewControllerState) {
    ...
    override func initializer() -> ChatControllerView {
        return ChatControllerView(frame: NSRect(x: _frameRect.minX, y: _frameRect.minY, width: _frameRect.width, height: _frameRect.height - self.bar.height), chatInteraction: chatInteraction)
    }
    ...
    func showChatThemeSelector() {
        ...
        self.themeSelector?._frameRect = NSRect(x: 0, y: self.frame.maxY, width: frame.width, height: 230)
        self.themeSelector?.loadViewIfNeeded()

        self.chatInteraction.update({ $0.updatedInterfaceState({ $0.withUpdatedThemeEditing(true) })})
    }

    func focusExistingMessage(_ message: Message) {
