    ...
    for (filter, count) in filtersWithCounts {
        ...
        case let .filter(_, title, _, data):
            ...
            if let color = data.color, showTags {
                ...
                image = generateImage(NSSize(width: 20, height: 20), contextGenerator: { size, ctx in
                ...
                if data.isShared {
                    image = generateImage(NSSize(width: 20 + 3 + sharedImage.backingSize.width, height: 20), contextGenerator: { size, ctx in
                        ctx.clear(size.bounds)
            ...
            entries.append(.custom(sectionId: sectionId, index: index, value: .none, identifier: _id_preset(filter), equatable: .init(filter), comparable: nil, item: { initialSize, _ in
...
func ChatListFiltersListController(context: AccountContext) -> InputDataController {
    ...
    let arguments = ChatListPresetArguments(context: context, openPreset: { filter, isNew in
        ...
        // Fenixuz: "Show Folder Tags" is a LOCAL postbox preference
        // (updateChatListFiltersDisplayTags makes no server call) — unlock it for everyone
        // instead of gating behind Premium.
            ...
            return context.account.postbox.transaction { transaction -> ([(ChatListFilter, Int)], Bool) in
                return (filters.list.map { filter -> (ChatListFilter, Int) in
                    ...
                    if let cachedValue = chatCountCache.with({ dict -> Int? in
                        ...
                        _ = chatCountCache.modify { dict in
                            var dict = dict
                            switch filter {
                            case .allChats:
                                break
    ...
    controller.afterTransaction = { controller in
        ...
        if range.location != NSNotFound {
            ...
            controller.tableView.resortController = TableResortController(resortRange: range, start: { _ in
