...
private final class ExportTokenOptionView: View {
    ...
    private let cap = View(frame: NSRect(x: 0, y: 0, width: 20, height: 20))
    ...
    func update(title: String, number: String) {
        ...
        setFrameSize(NSSize(width: cap.frame.width + 10 + text.layoutSize.width, height: max(cap.frame.height, 4 + text.layoutSize.height + 4)))
    }

    override func layout() {
        super.layout()

        cap.setFrameOrigin(NSPoint.zero)
        let offset: CGFloat = optionText.frame.width == 6 ? 7 : 6
        optionText.setFrameOrigin(NSPoint(x: offset, y: 2))
        textView.setFrameOrigin(NSPoint(x: cap.frame.maxX + 10, y: 2))
    }
...
final class Auth_TokenView: View {
    ...
    fileprivate let logoView = NSImageView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
    ...
    required init(frame frameRect: NSRect) {
        firstHelp = ExportTokenOptionView(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: 0))
        secondHelp = ExportTokenOptionView(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: 0))
        thridHelp = ExportTokenOptionView(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: 0))
        super.init(frame: frameRect)
    ...
    private func updateLottie() {
        // Fenixuz: app icon (phoenix) instead of the Telegram airplane on the QR login screen.
    ...
    func update(state: QRTokenState, isLoading: Bool) {
        ...
        if !isLoading {
            ...
            if let view = self.progressView {
                ...
                current.setFrameSize(NSSize(width: 40, height: 40))
    ...
    override func layout() {
        ...
        firstHelp.setFrameOrigin(NSPoint(x: 0, y: 0))
        secondHelp.setFrameOrigin(NSPoint(x: 0, y: firstHelp.frame.maxY + 10))
        thridHelp.setFrameOrigin(NSPoint(x: 0, y: secondHelp.frame.maxY + 10))
...
final class Auth_TokenController: GenericViewController<Auth_TokenView> {
    ...
    func update(_ token: AuthTransferExportedToken?, cancel: @escaping () -> Void) {
        ...
        _ = signal.start(next: { [weak self] _, generate in
