...
final class InputMessageEffectView: Control {
    ...
    private let gradient: RadialGradientView = RadialGradientView(frame: NSRect(x: 0, y: 0, width: 20, height: 20))
    init(account: Account, file: TelegramMediaFile, size: NSSize) {
        ...
        super.init(frame: NSSize(width: size.width, height: 20).bounds)
...
class ChatInputView: View, Notifable {
    ...
    init(frame frameRect: NSRect, chatInteraction: ChatInteraction) {
        ...
        self.contentView = View(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: frameRect.height))
        self._ts = View(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: .borderSize))
        self.attachView = ChatInputAttachView(frame: NSRect(x: 0, y: 0, width: chatInteraction.mode.customChatLink != nil ? 20 : 60, height: contentView.frame.height), chatInteraction: chatInteraction)
        self.attachView.isHidden = chatInteraction.mode.customChatLink != nil
        self.actionsView = ChatInputActionsView(frame: NSRect(x: contentView.frame.width - 100, y: 0, width: 100, height: contentView.frame.height), chatInteraction: chatInteraction)
        self.textView = UITextView(frame: NSRect(x: attachView.isHidden ? 0 : attachView.frame.width, y: 0, width: contentView.frame.width - actionsView.frame.width, height: contentView.frame.height), interactions: self.textInteractions)
    ...
    func updateInterface(with interaction: ChatInteraction) {
    ...
    override func updateLocalizationAndTheme(theme: PresentationTheme) {
        ...
        // Novagram: 12.9 turns the composer into a rounded floating bar with no
        // hairline separator above it. Gated on Liquid Glass so the classic
        // flush composer is untouched when it is off.
    ...
    private func updateMessageEffect(_ messageEffect: ChatInterfaceMessageEffect?, animated: Bool) {
        ...
        if let messageEffect {
            if self.messageEffect?.view.animateLayer.fileId != messageEffect.effect.effectSticker.fileId.id {
                ...
                let current = InputMessageEffectView(account: chatInteraction.context.account, file: messageEffect.effect.effectSticker._parse(), size: NSSize(width: 16, height: 16))
                current.userInteractionEnabled = true
                current.setFrameOrigin(NSPoint(x: frame.width - current.frame.width - 10, y: 5))
                ...
                if let fromRect = messageEffect.fromRect {
                    ...
                    let animationSize = NSSize(width: 200, height: 200)
                    ...
                    _ = (signal |> deliverOnMainQueue).startStandalone(next: { value in
                        ...
                        if let animation = value?.0 {
                            let player = LottiePlayerView(frame: NSRect(x: toRect.minX - animationSize.width / 2 - 50, y: toRect.minY - animationSize.height / 2 + 30, width: animationSize.width, height: animationSize.height))
    ...
    func needUpdateReplyMarkup(with state: ChatPresentationInterfaceState, _ animated: Bool) {
        if let keyboardMessage = state.keyboardButtonsMessage, let attribute = keyboardMessage.replyMarkup, state.isKeyboardShown || attribute.flags.contains(.persistent) {
            ...
            bottomView.contentView.scroll(to: NSPoint.zero)
        }
    }
    ...
    func needUpdateChatState(with state: ChatState, _ animated: Bool) {
        CATransaction.begin()
        if animated {
            inputDidUpdateLayout(animated: animated)
        }
        ...
        case let .block(string):
            if !string.isEmpty {
                let current = Control(frame: NSRect(x: 0, y: 0, width: frame.width, height: frame.height - 1))
        ...
        case let .recording(recorder):
            ...
            recordingPanelView = ChatInputRecordingView(frame: NSRect(x: 0, y: 0, width: frame.width, height: standart), chatInteraction: chatInteraction, recorder: recorder)
    ...
    func updateInput(_ state: ChatPresentationInterfaceState, prevState: ChatPresentationInterfaceState, animated: Bool = true, initial: Bool = false) {
    ...
    func updateAdditions(_ state: ChatPresentationInterfaceState, _ animated: Bool = true) {
    ...
    func updateAttachments(_ inputState: ChatPresentationInterfaceState, _ animated: Bool = true) {
        if let botMenu = inputState.botMenu, !botMenu.isEmpty, inputState.interfaceState.inputState.inputText.isEmpty {
            ...
            if let view = self.botMenuView {
                ...
                current = ChatInputMenuView(frame: NSRect(x: 0, y: 0, width: 60, height: 50))
        ...
        if let sendAsPeers = inputState.sendAsPeers, !sendAsPeers.isEmpty && inputState.state == .normal {
            ...
            if let view = self.sendAsView {
                ...
                current = ChatInputSendAsView(frame: NSRect(x: 0, y: 0, width: 50, height: 50))
    ...
    func updateLayout(size: NSSize, transition: ContainedViewLayoutTransition) {
        ...
        transition.updateFrame(view: contentView, frame: NSRect(x: 0, y: bottomInset, width: size.width, height: contentHeight))
        transition.updateFrame(view: bottomView, frame: NSRect(x: 20, y: chatInteraction.presentation.isKeyboardShown ? 0 : -bottomHeight, width: keyboardWidth, height: bottomHeight))
        ...
        if let view = messageEffect {
            transition.updateFrame(view: view, frame: NSRect(x: size.width - view.frame.width - 10, y: 5, width: view.frame.width, height: view.frame.height))
        }

        if let view = botMenuView {
            leftInset += view.frame.width
            transition.updateFrame(view: view, frame: NSRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height))
        }
        ...
        if let markup = replyMarkupModel, markup.hasButtons, let view = markup.view {
            ...
            transition.updateFrame(view: view, frame: NSRect(x: 0, y: 0, width: markup.size.width, height: markup.size.height))
            markup.layout(transition: transition)
        }

        if let current = self.blockText {
            transition.updateFrame(view: current, frame: CGRect(origin: .zero, size: NSSize(width: size.width, height: size.height - 1)))
            if let subview = current.subviews.first {
                transition.updateFrame(view: subview, frame: subview.centerFrame())
            }
        }

        transition.updateFrame(view: attachView, frame: NSRect(x: leftInset, y: 0, width: attachView.frame.width, height: attachView.frame.height))
        leftInset += attachView.frame.width

        let (textSize, textHeight) = self.textViewSize(size.width)

        let viewRect = NSRect(x: leftInset, y: 0, width: textSize.width, height: textSize.height)
        ...
        transition.updateFrame(view: _ts, frame: NSRect(x: 0, y: size.height - .borderSize, width: size.width, height: .borderSize))

        accessory.measureSize(size.width - 64)
        transition.updateFrame(view: accessory, frame: NSRect(x: 15, y: contentView.frame.maxY, width: size.width - 39, height: accessory.size.height))
        accessory.updateLayout(NSSize(width: size.width - 39, height: accessory.size.height), transition: transition)
    ...
    func textViewSize(_ width: CGFloat) -> (NSSize, CGFloat) {
        ...
        return (NSSize(width: w, height: min(max(height, textView.min_height), textView.max_height)), height)
    }
