...
private final class VideoAvatarProgressView: View {
    private let progressView = ProgressIndicator(frame: NSRect(x: 0, y: 0, width: 20, height: 20))
    required init(frame frameRect: NSRect) {
        super.init(frame: frameRect)
...
private final class VideoAvatarContainer: View {
    ...
    func updateWith(file: TelegramMediaFile, seekTo: TimeInterval?, peer: Peer, reference: PeerReference?, context: AccountContext) {
       // player.update(FileMediaReference.standalone(media: file), context: context)
        ...
        view.frame = NSRect(x: 2, y: 2, width: frame.width - 4, height: frame.height - 4)
        ...
        statusDisposable.set(statusSignal.start(next: { [weak self] status in
            ...
            default:
                if self?.progressView == nil, let frame = self?.frame {
                    let view = VideoAvatarProgressView(frame: NSRect(x: 2, y: 2, width: frame.width - 4, height: frame.height - 4))
...
class ChatTitleBarView: TitledBarView, InteractionContentViewProtocol {
    ...
    private let avatarControl: AvatarStoryControl = AvatarStoryControl(font: .avatar(12), size: NSSize(width: 36, height: 36))
    ...
    private let photoContainer: Control = Control(frame: NSRect(x: 0, y: 0, width: 36, height: 36))
    ...
    private func updateStoryState(_ story: PeerExpiringStoryListContext.State?, animated: Bool) {
        ...
        if self.story != story {
            if let storyState = story, !storyState.items.isEmpty {
                ...
                avatarControl.update(component: compoment, availableSize: NSSize(width: 30, height: 30), transition: transition)
            } else {
                avatarControl.update(component: nil, availableSize: NSSize(width: 36, height: 36), transition: transition)
            }
        }
        self.story = story
    ...
    private var mouseDownWindowFrame: NSRect?
    ...
    override func viewDidMoveToSuperview() {
        ...
        updateListeners()
        updateAnimatableContent()
        // the hosting NavigationBarView is only reachable once we are in the
        // hierarchy — turn its bar-wide glass off so the capsule is the only layer
        enclosingNavigationBar?.usesGlassBackground = !isLiquidGlassAvailable
    }
    ...
    @objc func updateAnimatableContent() {
    ...
    // Novagram: Liquid Glass capsule behind the title area (macOS 26+), matching
    // official Telegram 12.9 where the chat title is a floating rounded island and
    // the call/search buttons sit outside it. Purely additive — when Liquid Glass is
    // off or unsupported the view is never created and layout stays as upstream.
    ...
    /// Walks up to the `NavigationBarView` hosting this title view, so the capsule
    /// can turn off the bar-wide glass and stay the only glass layer.
    private var enclosingNavigationBar: NavigationBarView? {
        var next: NSView? = self.superview
        while let current = next {
            if let bar = current as? NavigationBarView {
                return bar
            }
    ...
    override func layout() {
        ...
        if let photo = inlineTopicPhotoLayer {
            photo.frame = NSRect(x: additionInset, y: floorToScreenPixels(backingScaleFactor, (containerView.frame.height - photo.frame.height) / 2), width: photo.frame.width, height: photo.frame.height)
        }
        ...
        if let capsule = glassCapsule {
            // stop the capsule before the leading edge of whichever trailing button
            // is visible, so the buttons stay outside the island like in 12.9
            var rightEdge = frame.width
            if !callButton.isHidden {
                rightEdge = callButton.frame.minX - Constants.glassCapsuleTrailingGap
            } else if !searchButton.isHidden {
                rightEdge = searchButton.frame.minX - Constants.glassCapsuleTrailingGap
            }
            let capsuleHeight = max(0, frame.height - Constants.glassCapsuleVerticalInset * 2)
            capsule.frame = NSRect(x: 0, y: Constants.glassCapsuleVerticalInset, width: max(0, rightEdge), height: capsuleHeight)
            capsule.cornerRadius = min(Constants.glassCapsuleCornerRadius, capsuleHeight / 2)
        }
        ...
        if let statusControl = statusControl, let titleRect = titleRect {
            statusControl.setFrameOrigin(NSPoint(x: titleRect.maxX + 2, y: titleRect.minY + 1))
        }

        if let statusControl = leftStatusControl, let titleRect = titleRect {
            statusControl.setFrameOrigin(NSPoint(x: titleRect.minX - statusControl.frame.width - 2, y: titleRect.minY + 1))
        }
        ...
        if let peerView = self.peerView {
            ...
            if let peer = peerViewMainPeer(peerView) {
                if peer.id == repliesPeerId {
                    ...
                    avatarControl.setSignal(generateEmptyPhoto(avatarControl.frame.size, type: .icon(colors: theme.colors.peerColors(5), icon: icon, iconSize: icon.backingSize.aspectFitted(NSSize(width: avatarControl.frame.size.width - 17, height: avatarControl.frame.size.height - 17)), cornerRadius: nil), bubble: false) |> map {($0, false)})
                } else if peer.id.isAnonymousSavedMessages {
                    let icon = theme.icons.chat_hidden_author
                    avatarControl.setSignal(generateEmptyPhoto(avatarControl.frame.size, type: .icon(colors: theme.colors.peerColors(5), icon: icon, iconSize: icon.backingSize.aspectFitted(NSSize(width: avatarControl.frame.size.width - 5, height: avatarControl.frame.size.height - 5)), cornerRadius: nil), bubble: false) |> map {($0, false)})
                    ...
                    avatarControl.setSignal(generateEmptyPhoto(avatarControl.frame.size, type: .icon(colors: theme.colors.peerColors(5), icon: icon, iconSize: icon.backingSize.aspectFitted(NSSize(width: avatarControl.frame.size.width - 15, height: avatarControl.frame.size.height - 15)), cornerRadius: nil), bubble: false) |> map {($0, false)})
                } else {
