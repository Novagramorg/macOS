...
private final class VoiceView: View, MediaView {
    let hood = View(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
    let durationView = DynamicCounterTextView()
    let waveformView = AudioWaveformView(frame: NSRect(x: 0, y: 0, width: 170, height: 20))
    var fireView: InlineStickerView?
    private let fireControl: FireTimerControl = FireTimerControl(frame: NSRect(x: 0, y: 0, width: 45, height: 45))
...
//        progress.fillColor = .clear
        sparkView.frame = NSRect(x: 60, y: 10, width: 100, height: 20)
    ...
    func songDidChangedState(song: APSongItem, for controller: APController, animated: Bool) {
        ...
        case let .paused(current, duration, _):
            ...
            self.sparksAnimator = ConstantDisplayLinkAnimator(update: { [weak self] in
                ...
                self.sparkView.update(position: NSPoint(x: 5 + width * (1 - progressValue), y: 18), sampleHeight: 5, color: self.activityBackground)

                self.waveformView.foregroundClipingView.setFrameSize(NSSize(width: width * (1 - progressValue), height: 20))
            })
    ...
    func set(media: TelegramMediaFile, isIncoming: Bool, context: AccountContext) {
        ...
        let fireView = InlineStickerView(account: context.account, file: LocalAnimatedSticker.single_voice_fire.file, size: NSSize(width: 30, height: 30), getColors: { _ in
    ...
    override func layout() {
        ...
        sparkView.frame = NSRect(x: hood.frame.maxX + 10, y: floorToScreenPixels((frame.height - 20) / 2) - 10, width: frame.width - (hood.frame.maxX + 10) - 10, height: 20)

        waveformView.frame = NSRect(x: hood.frame.maxX + 10, y: floorToScreenPixels((frame.height - 20) / 2) - 10, width: frame.width - (hood.frame.maxX + 10) - 10, height: 20)

        durationView.setFrameOrigin(NSPoint(x: hood.frame.maxX + 10, y: waveformView.frame.maxY + 4))
    }
}
...
private final class VideoView: View, MediaView {
    ...
    override func layout() {
        ...
        durationView.setFrameOrigin(NSPoint(x: 8, y: frame.height - durationView.frame.height))
    }
}
...
private final class SingleTimeMediaView: View {
    ...
    func update(context: AccountContext, message: Message) {
        ...
        if media.isVoice {
            let voiceView = VoiceView(frame: NSRect(x: 0, y: 0, width: 220, height: 60))
            addSubview(voiceView)
            self.mediaView = voiceView

            voiceView.set(media: media, isIncoming: isIncoming, context: context)
        } else {
            let videoView = VideoView(frame: NSRect(x: 0, y: 0, width: 250, height: 250))
        ...
        close.sizeToFit(NSSize(width: 20, height: 20))
...
final class SingleTimeMediaViewer: ModalViewController {
    ...
    override func viewDidLoad() {
        ...
        genericView.mediaView?.didFinish = { [weak self] _ in
            self?.close()
        }
