...
private final class AuthModalController: ModalController {
    ...
    override func measure(size: NSSize) {
        self.modal?.resize(with: NSSize(width: size.width, height: size.height), animated: false)
    }
}
...
final class UnauthorizedApplicationContext {
    ...
    init(window: Window, sharedContext: SharedAccountContext, account: UnauthorizedAccount, otherAccountPhoneNumbers: ((String, AccountRecordId, Bool)?, [(String, AccountRecordId, Bool)])) {
        ...
        if window.frame.height < window.minSize.height || window.frame.width < window.minSize.width {
            window.setFrame(NSRect(x: window.frame.minX, y: window.frame.minY, width: window.minSize.width, height: window.minSize.height), display: true)
            window.center()
        }
        ...
        rootController._frameRect = NSRect(x: 0, y: 0, width: window.frame.width, height: window.frame.height)
...
private final class ApplicationContainerView: View {
    ...
    required init(frame frameRect: NSRect) {
        splitView = SplitView(frame: NSRect(x: 0, y: 0, width: frameRect.width, height: frameRect.height))
    ...
    override func layout() {
        ...
        if let leftSideView = leftSideView {
            leftSideView.frame = NSRect(x: 0, y: 0, width: leftSidebarWidth, height: frame.height)
            splitView.frame = NSRect(x: leftSideView.frame.maxX, y: 0, width: frame.width - leftSideView.frame.maxX, height: frame.height)
        } else {
...
final class AuthorizedApplicationContext: NSObject, SplitViewDelegate {
    ...
    init(window: Window, context: AccountContext, launchSettings: LaunchSettings, callSession: PCallSession?, groupCallContext: GroupCallContext?, inlinePlayerContext: InlineAudioPlayerView.ContextObject?, folders: ChatListFolders?) {
        ...
        if !window.initFromSaver {
            window.setFrame(NSRect(x: 0, y: 0, width: 800, height: 650), display: true)
            window.center()
        }
        ...
        // ko'rinishi kerak. Settings (va boshqa) tab'larda yashiramiz.
        // applyLeftSidebarVisibility BOTH the folders-as-sidebar state AND the current
        // tab'ni tekshiradi, shuning uchun har tab almashganda qaytadan hisoblaymiz.
        leftController.onTabChanged = { [weak self] _ in
            self?.applyLeftSidebarVisibility(animated: true)
        }
        ...
        context.bindings = AccountContextBindings(rootNavigation: { [weak self] () -> MajorNavigationController in
            ...
            guard let `self` = self else {
                return EntertainmentViewController.init(size: NSSize.zero, context: context)
            }
            if self.entertainment == nil {
                self.entertainment = EntertainmentViewController(size: NSSize(width: 350, height: 350), context: self.context)
            }
       ...
       // var forceNotice:Bool = false
        ...
        self.loggedOutDisposable.set(context.account.loggedOut.start(next: { value in
            if value {
                _ = logoutFromAccount(id: accountId, accountManager: context.sharedContext.accountManager, alreadyLoggedOutRemotely: false).start()
...
//        }, with: self, for: .F, priority: .supreme, modifierFlags: [.command, .shift])
        ...
        appUpdateDisposable.set((context.account.stateManager.appUpdateInfo |> deliverOnMainQueue).start(next: { _ in
    ...
    private func updateLeftSidebar(with folders: ChatListFolders, layout: SplitViewState, animated: Bool) {
        ...
        if currentSidebar != previousSidebar {
            if !currentSidebar {
                ...
                controller._frameRect = NSRect(x: 0, y: 0, width: leftSidebarWidth, height: window.frame.height)
            ...
            foldersReadyDisposable.set(readySignal.start(next: { [weak self] _ in
                ...
                // Fenixuz: rail tab'ga bog'liq — helper Chats tab faolligini tekshiradi.
                self.applyLeftSidebarVisibility(animated: animated)
                if !self.window.isFullScreen, let screen = self.window.screen {
                    self.window.setFrame(NSRect(x: max(0, self.window.frame.minX - enlarge), y: self.window.frame.minY, width: min(self.window.frame.width + enlarge, screen.frame.width), height: self.window.frame.height), display: true, animate: false)
                }
    ...
    private func updateMinMaxWindowSize(animated: Bool) {
        ...
        window.minSize = NSSize(width: width, height: 550)

        if window.frame.width < window.minSize.width {
            window.setFrame(NSRect(x: max(0, window.frame.minX - (window.minSize.width - window.frame.width)), y: window.frame.minY, width: window.minSize.width, height: window.frame.height), display: true, animate: false)
        }
    }
    ...
    func splitViewShouldResize(at point: NSPoint) {
        if !FastSettings.isMinimisize {
            ...
            self.view.splitView.updateStartSize(size: NSSize(width: result, height: result), controller: leftController)
        }
