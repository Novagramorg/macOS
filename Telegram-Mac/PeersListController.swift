        ...
        case .connecting, .waitingForNetwork:
            if pref.enabled {
                ...
                if let view = self.connecting {
                    ...
                    current = ProgressIndicator(frame: focus(NSSize(width: 11, height: 11)))
...
private final class StatusView: Control {
    ...
    func update(_ peer: Peer, context: AccountContext, animated: Bool) {
        ...
        var interactiveStatus: Reactions.InteractiveStatus?
    ...
    private func playAnimation(_  status: Reactions.InteractiveStatus, context: AccountContext) {
        ...
        let play: (StatusView) -> Void = { [weak control] superview in
            ...
            let panel = Window(contentRect: NSRect(x: 0, y: 0, width: 160, height: 120), styleMask: [.fullSizeContentView], backing: .buffered, defer: false)
            ...
            let player = CustomReactionEffectView(frame: NSSize(width: 160, height: 120).bounds, context: context, fileId: fileId)
...
private final class WebappsControl: Control {
        ...
        func update(item: WebAppItem, context: AccountContext) {
            ...
            if item.data.external?.isSite == true {
                ...
                if case .instantView = item.data.unique {
                    current.nsImage = generateContextMenuInstantView(color: color, size: NSSize(width: 24, height: 24))
                } else  if let favicon = item.data.external?.favicon {
                    current.nsImage = favicon
                } else {
                    current.nsImage = generateContextMenuUrl(color: color, state: item.data.external, size: NSSize(width: 24, height: 24))
                }
    ...
    func update(webapps: [BrowserTabData], context: AccountContext, animated: Bool) {
        ...
        if items.count == 4 || items.count == 0 {
            ...
            size = NSSize(width: last.maxX, height: 24)
        }
...
private final class TitleView: Control {
    ...
    fileprivate func updateState(_ state: PeerListState, arguments: Arguments, maxWidth: CGFloat, animated: Bool) {
        ...
        if hasStatus, let peer = state.peer?.peer {
            ...
            if let view = self.premiumStatus {
                ...
                current = StatusView(frame: CGRect(origin: NSPoint(x: textView.frame.width + 4, y: (frame.height - 20) / 2), size: NSSize(width: 20, height: 20)))
...
private final class TitleForumView: Control {
    ...
    func update(_ state: PeerListState, forumData: PeerListState.ForumData, arguments: Arguments) {
        ...
        settings.sizeToFit(.zero, NSSize(width: 30, height: 30), thatFit: true)
...
class PeerListContainerView: Control {
    ...
    let backgroundView = View(frame: NSRect.zero)

    let tableView = TableView(frame: NSRect.zero, drawBorder: true)

    private let containerView = Control()
    private let statusContainer = Control()

    let searchView: SearchView = SearchView(frame: NSRect(x: 10, y: 0, width: 0, height: 0))
    ...
    var searchViewRect: NSRect {
        ...
        return NSRect(x: 0, y: max(0, y), width: frame.width, height: frame.height - y)
    }
    ...
    var openProxy: ((Control) -> Void)?
    var openCurrentProfile: ((PeerId) -> Void)?
    var openStatus: ((Control) -> Void)? {
        didSet {
    ...
    fileprivate func updateState(_ state: PeerListState, arguments: Arguments, animated: Bool) {
        ...
        if hasCompose {
            ...
            if let view = self.compose {
                ...
                current = ImageButton(frame: NSRect(x: frame.width - 10 - 40, y: (statusHeight - 30)/2.0, width: 40, height: 30))
        ...
        if hasForumTitle, let forumData = state.forumPeer {
            ...
            if let view = self.forumTitle {
                ...
                current = TitleForumView(frame: NSRect(x: 0, y: 0, width: frame.width, height: 50))
        ...
        let componentSize = NSSize(width: 40, height: 30)

        var controlPoint = NSPoint(x: frame.width - 10, y: floorToScreenPixels(backingScaleFactor, (containerView.frame.height - componentSize.height)/2.0))
        ...
        if let item = self.storiesItem {
            ...
            if let view = self.storiesView {
                ...
                current = StoryListChatListRowView(frame: NSRect(x: 0, y: 50, width: frame.width, height: item.height))
        ...
        if let item = self.foldersItem {
            ...
            if let view = self.foldersView {
                ...
                current = ChatListRevealView(frame: NSRect(x: 0, y: navigationHeight - item.height, width: frame.width, height: item.height))
                containerView.addSubview(current, positioned: .below, relativeTo: statusContainer)
...
//        self.searchView.updateTags(tags, theme.search.searchImage)
        ...
        if state.mode.groupId == .archive || (state.selectedForum != nil && state.splitState != .minimisize) || state.mode.isForumLike  || state.appear == .short {
            ...
            if let view = self.backButton {
                ...
                current = ImageButton(frame: NSRect(x: 10, y: 10, width: 40, height: 30))
            ...
            current.sizeToFit(.zero, NSSize(width: 40, height: 30), thatFit: true)
        ...
        if let webapps = state.webapps, !webapps.isEmpty, state.mode.groupId == .root, state.splitState != .minimisize, !hasForumTitle || state.forumPeer == nil {
            ...
            if let view = self.webapps {
                ...
                current = WebappsControl(frame: NSRect(x: 0, y: 0, width: 24, height: 24))
            ...
            if isNew {
                current.setFrameOrigin(NSPoint(x: 10, y: floorToScreenPixels((50 - current.frame.height) / 2)))
                if animated {
                    current.layer?.animateAlpha(from: 0, to: 1, duration: 0.2)
                }
            }
        ...
        if state.isContacts, self.webapps == nil {
            ...
            if let view = self.contactsSort {
                ...
                current = TextButton(frame: NSRect(x: 10, y: 10, width: 40, height: 30))
            ...
            current.sizeToFit(NSSize(width: 10, height: 15))
    ...
    func updateScroller(animated: Bool) {
        ...
        if !state.isContacts, isShown {
            ...
            if let view = self.scrollerView {
                ...
                current.setFrameOrigin(NSPoint(x: frame.width - current.frame.width - 10, y: frame.height - current.frame.height - 10))
                addSubview(current)
    ...
    private(set) var delta: CGFloat?

    func updateSwipingState(_ state: SwipeState, controller: ViewController) {
        let transition: ContainedViewLayoutTransition
        switch state {
    ...
    fileprivate func updateGhostButton(animated: Bool) {
        ...
        if visible {
            ...
            if let view = self.ghost {
                ...
                current = ImageButton(frame: NSRect(x: 0, y: 0, width: 40, height: 30))
                current.layer?.cornerRadius = .cornerRadius
                current.autohighlight = false
                current.animates = false
                self.ghost = current
            ...
            // Fenixuz: the ghost asset is portrait ~455×491 and stored UPRIGHT, so render it small
            // (24×26, aspect preserved) WITHOUT the vertical flip — flipVertical: true puts it upside
            // down. Result: an upright, properly-sized ghost centered like the native header buttons.
            let ghostSize = NSSize(width: 24, height: 26)
            let icon: CGImage?
            if FenixuzGhostMode.isActive {
                let img = NSImage(named: "FenixGhostActive"); img?.size = ghostSize
                icon = img?.precomposed(flipVertical: false, scale: System.backingScale)
            } else {
                let img = NSImage(named: "FenixGhostInactive"); img?.size = ghostSize
                icon = img?.precomposed(theme.colors.grayIcon, flipVertical: false, scale: System.backingScale)
            }
            if let icon = icon {
    ...
    func updateLayout(_ size: NSSize, transition: ContainedViewLayoutTransition) {
        ...
        let searchRect = NSRect(x: 10, y: searchY, width: (size.width - 10 * 2), height: componentSize.height)
        ...
        var controlPoint = NSPoint(x: containerSize.width - 14, y: floorToScreenPixels(backingScaleFactor, (statusHeight - componentSize.height)/2.0))
        ...
        if let view = forumTitle {
            transition.updateFrame(view: view, frame: CGRect(origin: .zero, size: NSSize(width: size.width, height: 50)))
        ...
        if let view = self.backButton {
            if state.splitState == .minimisize {
                ...
                let rect = NSRect(x: 10, y: 10, width: 40, height: searchRect.height)
        ...
        if let view = self.webapps {
            transition.updateFrame(view: view, frame: CGRect(origin: NSPoint(x: 10, y: floorToScreenPixels((50 - view.frame.height) / 2)), size: view.frame.size))
        }

        if let view = self.scrollerView {
            transition.updateFrame(view: view, frame: NSRect(x: size.width - view.frame.width - 10, y: size.height - view.frame.height - 10, width: view.frame.width, height: view.frame.height))
        }
...
//        }
        ...
        if let storiesItem = storiesItem, let view = storiesView {
            let size = NSSize(width: size.width, height: storiesItem.height)

            var rect = CGRect(origin: NSPoint(x: storyX, y: 10 + storiesItem.getInterfaceState().progress * 40), size: size)
        ...
        if let foldersItem = foldersItem, let view = foldersView {
            let controlSize = NSSize(width: size.width, height: foldersItem.height)

            let rect = CGRect(origin: NSPoint(x: 0, y: navigationHeight - controlSize.height), size: controlSize)
    ...
    func updateDownloads(_ state: DownloadsSummary.State, context: AccountContext, arguments: DownloadsControlArguments, animated: Bool) {
        if !state.isEmpty {
            ...
            if let view = self.downloads {
                ...
                current = DownloadsControl(frame: NSRect(x: 0, y: frame.height - 30, width: frame.width - .borderSize, height: 30))
                self.downloads = current
                addSubview(current, positioned: .above, relativeTo: self.tableView)
                if animated {
                    current.layer?.animateAlpha(from: 0, to: 1, duration: 0.2)
                    current.layer?.animatePosition(from: NSPoint(x: current.frame.minX, y: current.frame.maxY), to: current.frame.origin)
                }
            }
            ...
            self.downloads = nil
            performSubviewPosRemoval(view, pos: NSPoint(x: 0, y: frame.maxY), animated: true)
        }
    }
...
private class SearchContainer: Control {
    ...
    init(frame frameRect: NSRect, searchView: NSView) {
        ...
        self.tagsView = .init(frame: NSRect(x: 0, y: -10, width: frameRect.width, height: 40))
        super.init(frame: frameRect)
        addSubview(tagsView)
    ...
    override func layout() {
        super.layout()
        tagsView.frame = NSRect(x: 0, y: -10, width: frame.width, height: 40)
        self.searchView.frame = NSRect(x: 0, y: tagsView.frame.maxY, width: frame.width, height: frame.height - tagsView.frame.maxY)
    }
}
...
class PeersListController: TelegramGenericViewController<PeerListContainerView>, TableViewDelegate {
    ...
    private(set) var searchSection: SectionViewController?
    ...
    func updateState(_ f: (PeerListState) -> PeerListState) {
    ...
    override func viewDidLoad() {
        ...
        genericView.tableView.addScroll(listener: .init(dispatchWhenVisibleRangeUpdated: false, { [weak self] _ in
        ...
        genericView.openStatus = { control in
            ...
            if let peer = peer as? TelegramUser {
                ...
                if control.popover == nil {
                    showPopover(for: control, with: PremiumStatusController(context, callback: callback, peer: peer), edge: .maxY, inset: NSPoint(x: -80, y: -35), static: true, animationMode: .reveal)
                }
            }
        }
...
//                    searchController.defaultQuery = self.genericView.searchView.query
                ...
                _ = signal.start(next: { [weak searchSection, weak self] _ in
                    if let searchSection = searchSection, let self {
                        ...
                        if animated {
                            ...
                            container.layer?.animatePosition(from: NSPoint(x: rect.minX, y: rect.minY + 15), to: rect.origin, duration: 0.4, timingFunction: .spring)
                        } else {
    ...
    private func hideSearchController(animated: Bool) {
        ...
        if let searchSection = self.searchSection, let container = searchContainer {
            ...
            container._change(opacity: 0, animated: animated, duration: 0.25, timingFunction: .spring, completion: { [weak container] _ in
    ...
    private func updateHighlight(_ peerId: PeerId?) {
        switch self.mode {
        case .forum, .savedMessagesChats:
            break
        default:
    ...
    override func updateSwipingState(_ state: SwipeState, controller: ViewController, isPrevious: Bool) {
    ...
    private var initFromEvent: Bool?
